...
def makepage(suffix, definitions, infinitive, root_kanji, root_hiragana):
    ...
    lines = [
        ...
        f"{{{{ja-mot|{new_page_title}|{new_hira}}}}} {{{{ja-pron|{new_hira}}}}} {{{{ja-verbe-ichidan|{infinitive}}}}}"
    ]
...
for i, line in enumerate(testpage.text.splitlines()):
    if toggle_first_tab:
        if line.startswith('{{ja-trans'):
            ...
            roothiragana = fullhiragana.split("る", 1)[0]
            rootkanji = testpage.title().split("る", 1)[0]
